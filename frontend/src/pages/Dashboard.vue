...
import { route } from '@/lib/route'; // Using centralized route helper
...
const expiringSoon = ref<PantryItem[]>([]);
const upcomingMeals = ref<MealPlan[]>([]);
const stats = ref({
    total_recipes: 0,
    total_collections: 0,
...
onMounted(async () => {
    try {
        ...
        recentRecipes.value = response.data.recentRecipes || [];
        pantryCount.value = response.data.pantryCount || 0;
        expiringSoon.value = response.data.expiringSoon || [];
        upcomingMeals.value = response.data.upcomingMeals || [];
        stats.value = response.data.stats || { total_recipes: 0, total_collections: 0, total_shopping_lists: 0 };
    } catch (error) {
        console.error('Failed to fetch dashboard data:', error);
        // Fallback Mock Data so the user sees the UI even without Auth
        recentRecipes.value = [
            { id: 1, title: 'Mock Spaghetti Carbonara', description: 'Creamy pasta', image: null, cuisine: 'Italian' }
        ];
        stats.value = {
            total_recipes: 5,
            total_collections: 2,
